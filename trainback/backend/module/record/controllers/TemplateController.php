...
use common\models\ProbationRecord4;
use common\models\ProbationRecord5;
use common\models\ProbationRecord6;
...
class TemplateController extends Controller{
    ...
    public function actionSubmitrecord3(){
    	$request = \Yii::$app->request;
    	$username=$request ->post('username');
    	$school_name=$request ->post('school_name');
    	...
    	$query=(new Query())
    	      ->select('*')
    	      ->from('probation_record4')
    	      ->andWhere(['username'=>$username])
    	      ->andWhere(['date'=>$date])
    	      ->andWhere(['schoolname'=>$school_name])
    	...
    	if($query==null){
    		...
    		$currentId=(new Query())
                  ...
                  ->max('id');
            $id=$currentId+1;
            $result=\Yii::$app->db->createCommand()->insert('probation_record4',
            	[
            	'id'=>$id,
            	'username'=>$username,
            	'schoolname'=>$school_name,
    ...
    public function actionSubmitrecord4(){
    	//教学设计部分
    	$request = \Yii::$app->request;
     	$username=$request ->post('username');
     	$date=$request ->post('date');
     	...
     	$classform=$request ->post('classform');
     	$teachTime=$request ->post('teachTime');
     	$teachContent=$request ->post('teachContent');
     	$contentAnalyse=$request ->post('contentAnalyse');
     	$objectAnalyse=$request ->post('objectAnalyse');
     	$objections=$request ->post('objections');
     	$difficulties=$request ->post('difficulties');
     	$strategies=$request ->post('strategies');
     	$process=$request ->post('process');
        $writing=$request ->post('writing');
     	$reflect=$request ->post('reflect');
     	$submitTime=$request ->post('submitTime');
     	//检测是否重复提交
    	$query=(new Query())
    	      ->select('*')
    	...
    	if($query==null){
    		...
    		$currentId=(new Query())
                  ...
                  ->max('id');
            $id=$currentId+1;
            $result=\Yii::$app->db->createCommand()->insert('probation_record5',
            	[
            	'id'=>$id,
            	'username'=>$username,
            	'date'=>$date,
     ...
     public function actionSubmitrecord5(){
     	$request = \Yii::$app->request;
     	$username=$request ->post('username');
     	$date=$request ->post('date');
     	$weekday=$request ->post('weekday');
     	$section=$request ->post('section');
     	$directTutor=$request ->post('directTutor');
     	$content1=$request ->post('content1');
     	$content2=$request ->post('content2');
    	...
    	if($query==null){
    		$currentId=(new Query())
                  ...
                  ->max('id');
            $id=$currentId+1;
            $result=\Yii::$app->db->createCommand()->insert('probation_record6',
            	[
            	'id'=>$id,
            	'username'=>$username,
            	'date'=>$date,
...
//先获取数据
   ...
   public function actionExportpdf3(){
   	//获取数据
		$request = \Yii::$app->request;
		$time=$request ->post('time');
		$username=$request ->post('username');
    ...
    $pdf->SetSubject('student_records'); //设置主题
    $pdf->SetKeywords('student,probation,record'); //设置关键词
    $pdf->SetHeaderData(0, PDF_HEADER_LOGO_WIDTH, '见习——教研活动见习记录', $detail['id'], array(0, 64, 255), array(0, 64, 128)); 
    //设置头部,比如header_logo，header_title，header_string及其属性
    $pdf->setFooterData(array(0, 64, 0), array(0, 64, 128));
    $pdf->setHeaderFont(Array("droidsansfallback", '', 12));
    $pdf->setFooterFont(Array('droidsansfallback', '', 12));
    ...
    $pdf->setFontSubsetting(true); //设置默认字体子集模式
    $pdf->AddPage(); //增加一个页面
    $pdf->SetFont('droidsansfallback' , 'B', 14);//设置字体
    $pdf->Cell(0, 10, '见习学校:', 0, 1, 'L');
    $pdf->Write(0, $detail['schoolname'], '', 0, 'L', true, 0, false, false, 0);
   ...
   public function actionExportpdf5(){
    ...
    $pdf->SetSubject('student_records'); //设置主题
    $pdf->SetKeywords('student,probation,record'); //设置关键词
    $pdf->SetHeaderData(0, PDF_HEADER_LOGO_WIDTH, '见习——见习生试讲听课记录', $detail['id'], array(0, 64, 255), array(0, 64, 128));
    //设置头部,比如header_logo，header_title，header_string及其属性
    $pdf->setFooterData(array(0, 64, 0), array(0, 64, 128));
    $pdf->setHeaderFont(Array("droidsansfallback", '', 12));
    $pdf->setFooterFont(Array('droidsansfallback', '', 12));
    ...
    $pdf->Write(0, $detail['section'], '', 0, 'L', true, 0, false, false, 0);
    $pdf->Cell(0, 10, '学院指导教师:', 0, 1, 'L');
    $pdf->Write(0, $detail['directTutor'], '', 0, 'L', true, 0, false, false, 0);
     $pdf->AddPage(); //增加一个页面
    $pdf->Cell(0, 10, '听课记录:', 0, 1, 'L');
    $pdf->Write(0, $detail['content1'], '', 0, 'L', true, 0, false, false, 0);
     $pdf->AddPage(); //增加一个页面
    $pdf->Cell(0, 10, '评价与建议:', 0, 1, 'L');
    $pdf->Write(0, $detail['content2'], '', 0, 'L', true, 0, false, false, 0);
     $pdf->AddPage(); //增加一个页面
    $pdf->Cell(0, 10, '教师指导意见记录:', 0, 1, 'L');
