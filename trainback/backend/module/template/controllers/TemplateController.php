...
class TemplateController extends Controller{
  ...
  public function actionUploadstu(){
    //实习部分学生模板
    $request = \Yii::$app->request;
    $username=$request->post('username');//模板上传前，先获取需要的变量
    $kind=$request->post('kind');
...
//实习部分校内教师模板
  public function actionUploadtea(){
    $request = \Yii::$app->request;
    $username=$request->post('username');//模板上传前，先获取需要的变量
    $kind=$request->post('kind');
    $menu=str_replace('\\','/',Yii::$app->basePath);
  ...
  public function actionUploadtemplate2(){
    //校外教师模板上传
    $request = \Yii::$app->request;
    $username=$request->post('username');//模板上传前，先获取需要的变量
    $kind=$request->post('kind');
  ...
  public function actionStutemplate(){
    //获取实习部分学生用模板
        $request = \Yii::$app->request;
        $username=$request->post('username');
        $data=(new Query())
  ...
  public function actionTeatemplate(){
    ...
    public function actionTeatemplate1(){
    //实习部分校内教师模板获取
     $request = \Yii::$app->request;
     $username=$request->post('username');
     $data=(new Query())
  ...
  public function actionTuttemplate(){
    //获取所有校外老师用模板
    $request = \Yii::$app->request;
    $username=$request->post('username');
    $data=(new Query())
