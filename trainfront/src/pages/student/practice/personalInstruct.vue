  ...
  <div>
    ...
    <div class="seeDetail" v-show="flag">
      <table>
        ...
        <tr v-for="(item,index) in instructList">
          <td>{{index+1}}</td>
          <td>{{item.filename}}</td>
          <td>{{item.submitTime}}</td>
          <td>
          <span style="cursor: pointer">
          <el-button type="primary" icon="el-icon-search" circle @click="showInstruct(item.rId)"></el-button>
          </span>
          </td>
        </tr>
      </table>
