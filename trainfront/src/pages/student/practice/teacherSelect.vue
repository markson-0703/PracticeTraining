...
<!--          选择本校导师-->
          ...
          <table id="teaStastics">
            ...
            <tr v-for="(item,index) in chooseList">
...
<!--          选择实习学校及其对应的老师-->
        ...
        <div class="show-result" v-show="showResult">
          ...
          <p v-if="this.ischoosen==1">{{this.finaltutor}}老师审核成功,你的实习地点为{{this.finalsite}}</p>
...
<script>
    export default {
        ...
        methods:{
            TipShow(){
                //判断学生是否已经申请了导师，则相应界面显示
                let that = this
                that.$http.post('/yii/practice/choose/ischoose',{
            ...
            TeaData(){
                ...
                that.$http.post('/yii/practice/practice/teadata',{page:that.currentpage})
                    .then((res)=>{
                        console.log(res.data)
                        that.chooseList=res.data.data[0]
            ...
            SchoolData(){
                //获取学校数据
                let that = this
                that.$http.post('/yii/practice/practice/getschool',{page:that.currentpage})
                    .then((res)=>{
                        console.log(res.data)
            ...
            TutorData(site){
                //获取实习点相应的负责老师
                let that = this
                that.$http.post('/yii/practice/choose/gettutdata',{
                    site:site
                }).then((res)=>{
                    console.log(res.data)
            ...
            showMember(){
                //显示小组成员
                let that = this
                that.$http.post('/yii/practice/choose/teammember',{
                    username:this.username
                }).then((res)=>{
                    console.log(res.data)
            ...
            getSiteData(name,jno){
                //获取导师相应的实习点
                let that = this
                that.$http.post('/yii/practice/choose/getsite',{
                    tName:name,
                    job_num:jno
                }).then((res)=>{
                    console.log(res.data)
            ...
            submit(){
                //选择完实习点和导师后提交
                let that = this
                that.$http.post('/yii/practice/choose/tutorarrange',{
                    username:that.username,
                    sno:that.sno,
                    sName:that.sName,
                    tutor:that.tutor,
            ...
            searchSite(){
                //搜索地址信息
                let that = this
                that.$http.post('/yii/practice/practice/queryplace',{
                    page:that.currentpage,
