<template>
    <div class="display" id="maintop">
      <el-tabs  :tab-position="tabPosition" v-model="activeName" @tab-click="handleClick" style="height: fit-content;">
        ...
        <el-tab-pane label="教研活动见习记录" name="fourth">
          <div class="record4">
            <div class="detail" id="pdfDom">
              <el-form ref="form" :model="recordForm4" :info="printDetail3" label-width="80px">
                <el-form-item label="见习学校:" style="width:400px;" prop="school_name">
                  <el-input v-model="recordForm4.school_name"></el-input>
                </el-form-item>
                <el-form-item label="年级:" style="width:400px;" prop="grade">
                  <el-input v-model="recordForm4.grade"></el-input>
                </el-form-item>
                <el-form-item label="日期:" style="width:400px;" prop="date">
                  <el-date-picker
                    v-model="recordForm4.date"
                    type="date"
                    placeholder="选择日期"
                ...
                <el-form-item label="课题:" style="width:400px;" prop="theme">
                  <el-input v-model="recordForm4.theme"></el-input>
                </el-form-item>
                <el-form-item label="教研活动见习记录:" prop="content1">
                  <quill-editor
                    v-model="recordForm4.content1"
                    ref="myQuillEditor"
                    :options="custom"
    ...
    export default {
        ...
        methods:{
           ...
           this.$confirm('请先确保无误后再上传?', '提示', {
                 ...
                 let that = this
               //去除富文本编辑器的p标签
               that.recordForm4.content1= that.recordForm4.content1.replace(/<[^>]+>/g, "")
               that.recordForm4.content2= that.recordForm4.content2.replace(/<[^>]+>/g, "")
               that.$http.post('/yii/record/template/submitrecord3',{
                   username:that.username,
                   school_name:that.recordForm4.school_name,
                   grade:that.recordForm4.grade,
                   date:that.recordForm4.date,
                   theme:that.recordForm4.theme,
                   content1:that.recordForm4.content1,
                   content2:that.recordForm4.content2,
            ...
            createPdf(time){
                //生成PDF
                let that = this
                that.$http.post('/yii/record/template/exportpdf3',{
                    time:time,
                    username:that.username
                }).then((res)=>{
                    console.log(res.data)
