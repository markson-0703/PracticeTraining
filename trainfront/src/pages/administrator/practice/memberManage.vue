    ...
    <el-tab-pane label="校内教师信息管理" name="first">
      <div class="display1">
        <div class="search">
          ...
          <button class="btn3" v-on:click="findTea()">搜索</button>
          <button class="btn3" type="text" @click="newTea()">添加</button>
          <!--          添加教师信息-->
          ...
          <el-dialog title="修改教师信息" :visible.sync="dialogFormVisible">
            ...
            <div slot="footer" style="align-content: center" class="dialog-footer">
              <el-button type="primary" @click="saveTea()">更新</el-button>
        ...
        <table id="teacherStastics">
          ...
          <tr v-for="(item,index) in teacherList">
            ...
            <td>
              <span style="cursor: pointer" @click="editTea(item.username)"><el-button type="primary" icon="el-icon-edit" circle></el-button></span>
    ...
    <el-tab-pane label="学生信息管理" name="second">
      <div class="display1">
        <div class="search">
          ...
          <button class="btn3" v-on:click="findStu()">搜索</button>
    ...
    export default {
        ...
        data(){
            return{
                ...
                dialogFormVisible:false,//校内教师修改
        ...
        methods:{
            ...
            teacherData(){
                //校内教师数据获取
                let that = this
                that.$http.post('/yii/practice/practice/teadata',{page:that.currentpage})
                    .then((res)=>{
            ...
            editTea(uName){
               //修改校内教师信息
                let that =this
                that.$http.post('/yii/practice/practice/edittea',{username:uName})
                    .then((res)=>{
                        console.log(res.data)
            ...
            saveS(){
                //保存修改的学生信息
                let that = this
                that.$http.post('/yii/practice/practice/alterstu',{
            ...
            resetStu(){
                //重置学生数据
                this.stuForm.sName='';this.stuForm.sno='';this.stuForm.sex='';this.stuForm.cId='';this.stuForm.className='';
                this.stuForm.majorId='';this.stuForm.majorName='';this.stuForm.bornDate='';this.stuForm.phone='';this.stuForm.email='';
            },
            deleteTea(uName){
                //删除校内教师信息
                this.$confirm('此操作将永久删除该教师用户, 是否继续?', '提示', {
                    ...
                    let that = this
                    that.$http.post('/yii/practice/practice/deletetea',{username:uName})
                        .then((res)=>{
                            console.log(res.data)
                            if(res.data.message=="success"){
                                that.teacherData()
            ...
            findTea(){
                //搜索校内教师
                let that =this
                that.$http.post('/yii/practice/practice/findtea',{
            ...
            searchTut(){
                //查找校外教师
                    let that = this
                    that.$http.post('/yii/practice/practice/findtut',{
                        page:that.currentpage,
                        tName:that.inputtut
                    }).then((res)=>{
                        console.log(res.data)
            ...
            submitTut(){
                ...
                this.$confirm('此操作所添加用户的密码默认为123456,是否继续操作?', '提示', {
                    ...
                    if(that.tutForm.username!=''&&that.tutForm.tName!=''&&that.tutForm.job_num!=''&&that.tutForm.school_name!=''&&that.tutForm.contactPhone!=''&&that.tutForm.email!=''&&that.tutForm.rank!='') {
                        that.$http.post('/yii/practice/practice/inserttut', {
            ...
            saveTut(){
                //保存修改的校外信息
                let that = this
                that.$http.post('/yii/practice/practice/altertut',{
                    username:that.tutForm.username,
                    school_name:that.tutForm.school_name,
                    phone:that.tutForm.contactPhone,
                    email:that.tutForm.email,
            ...
            getEdittut(uName){
               //编辑校外教师
                let that =this
                that.$http.post('/yii/practice/practice/edittut',{username:uName})
                    .then((res)=>{
                        console.log(res.data)
            ...
            deleteTut(uName){
                this.$confirm('此操作将永久删除该教师用户, 是否继续?', '提示', {
                    ...
                    let that = this
                    that.$http.post('/yii/practice/practice/deletetut',{username:uName})
                        .then((res)=>{
                            console.log(res.data)
                            if(res.data.message=="success"){
                                that.tutorData()
            ...
            tutorData(){
                //获取校外教师信息
                let that = this
                that.$http.post('/yii/practice/practice/tutdata',{page:that.currentpage})
                    .then((res)=>{
                        console.log(res.data)
                        that.tutorList=res.data.data[0]
                        that.totalpage=res.data.data[1]
            ...
            submitTea(){
                ...
                this.$confirm('此操作所添加用户的密码默认为123456,是否继续操作?', '提示', {
                    ...
                    if(that.teaForm.username!=''&&that.teaForm.tName!=''&&that.teaForm.job_num!=''&&that.teaForm.contactPhone!=''&&that.teaForm.email!=''&&that.teaForm.rank!='') {
                        that.$http.post('/yii/practice/practice/inserttea', {
                            username: that.teaForm.username,
            ...
            submitStu(){
                ...
                this.$confirm('此操作所添加用户的密码默认为123456,是否继续操作?', '提示', {
                    ...
                    {
                        that.$http.post('/yii/practice/practice/insertstu',{
                            username:that.stuForm.username,
                            name:that.stuForm.sName,
                            sno:that.stuForm.sno,
                            sex:that.stuForm.sex,
            ...
            studentData(){
                //学生数据
                let that = this
                that.$http.post('/yii/practice/practice/studata',{page:that.currentpage})
                    .then((res)=>{
                        console.log(res.data)
            ...
            findStu(){
                //搜索学生
                let that = this
                that.$http.post('/yii/practice/practice/findstu',{
                    page:that.currentpage,
                    name:that.inputstu
                }).then((res)=>{
                    console.log(res.data)
            ...
            addStu(){
                //添加学生
                this.dialogFormVisible3=true;this.stuForm.username='';this.stuForm.sName='';this.stuForm.sno='';
                this.stuForm.sex='';this.stuForm.cId='';this.stuForm.className='';this.stuForm.majorId='';
                this.stuForm.majorName='';this.stuForm.bornDate='';this.stuForm.phone='';this.stuForm.email='';
            },
            getEditstu(uName){
                //修改学生数据
                let that = this
                that.$http.post('/yii/practice/practice/editstu',{username:uName})
                    .then((res)=>{
                        console.log(res.data)
            ...
            deleteStu(uName){
                ...
                this.$confirm('此操作将永久删除该学生用户, 是否继续?', '提示', {
                    ...
                    let that = this
                    that.$http.post('/yii/practice/practice/deletestu',{username:uName})
                        .then((res)=>{
                            console.log(res.data)
                            if(res.data.message=="success"){
                                that.studentData()
            ...
            importfxx(obj,flag){
                ...
                FileReader.prototype.readAsBinaryString = function (f){
                    ...
                    reader.onload = function (e) {
                        ...
                        if (flag == 1) {
                            _this.$http.post('/yii/practice/practice/importexcel', data).then(body => {
                                console.log(body)
                                alert('导入成功')
                                _this.teacherData()
                            })
                        } else if (flag == 2) {
                            _this.$http.post('/yii/practice/practice/importexcel1', data).then(body => {
                                alert('导入成功')
                                _this.studentData()
                            })
                        } else if (flag == 3) {
                            _this.$http.post('/yii/practice/practice/importexcel2', data).then(body => {
                                alert('导入成功')
                                _this.tutorData()
                            })
                        }
