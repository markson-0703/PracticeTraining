<template>
  <el-tabs v-model="activeName" type="card" @tab-click="getValue">
    ...
    export default {
        ...
        methods:{
            ...
            checkStatus(sno){
                ...
                console.log(sno)
                let that = this
                that.$http.post('/yii/probation/infomation/getstatus',{
                    sno:sno
                }).then((res)=>{
            ...
            setLeader(sno){
                //设置为组长
                let that = this
                that.$http.post('/yii/probation/verify/setleader',{
                    sno:sno,
                }).then((res)=>{
                    console.log(res.data)
            ...
            removeLeader(sno){
                ...
                this.$confirm('是否要移除该用户的组长身份?', '提示', {
                    ...
                    let that = this
                    that.$http.post('/yii/probation/verify/removeleader', {
                        sno: sno,
                    }).then((res) => {
                        console.log(res.data)
                        if(res.data.message=="success"){
            ...
            agree(sno){
                //导师审核
                let that = this
                that.$http.post('/yii/probation/verify/checkstu',{
                    sno:sno,
            ...
            getArrangeInfo(){
                //获取分配信息，一共多少的见习点
                let that = this
                that.$http.post('/yii/probation/infomation/getproarr',{
                    username:that.username
                }).then((res)=>{
                        console.log(res.data)
                        that.arrangeList=res.data.data
                    // let list = that.arrangeList
                    // for(let i=0;i<list.length;i++)
                    // {
                    //     console.log(list[i].site)
                    // }
